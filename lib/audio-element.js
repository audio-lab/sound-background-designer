...
function AudioElement (options) {
	...
	self.processor = ctx.createScriptProcessor();
	self.processor.onaudioprocess = function (e) {
		var iData1 = e.inputBuffer.getChannelData(0);
		var iData2 = e.inputBuffer.getChannelData(1);
		var oData1 = e.outputBuffer.getChannelData(0);
		var oData2 = e.outputBuffer.getChannelData(1);

		//slice prerendered data to processor
		oData1.set(iData1);
		oData2.set(iData2);
	};
...
AudioElement.prototype.regenerate = function () {
	...
	var freqRatio = self._source.length / samplesPerPeriod;
	var data1 = self.source.buffer.getChannelData(0);
	var data2 = self.source.buffer.getChannelData(1);
	...
	for (;;) {
		data1[i] = self._source[~~n % self._source.length];
		data2[i] = data1[i];
